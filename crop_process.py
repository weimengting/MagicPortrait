...
import data_utils.datasets_faceswap as datasets_faceswap
...
def crop_a_directory(args):
    os.makedirs(args.video_imgs_dir, exist_ok=True)
    imgs = sorted(os.listdir(args.video_path))
    first_img_path = os.path.join(args.video_path, imgs[0])
    im_pil_sor = Image.open(first_img_path).convert("RGB")
    face_info_sor = app.get(cv2.cvtColor(np.array(im_pil_sor), cv2.COLOR_RGB2BGR))
    assert len(face_info_sor) >= 1, 'The input image must contain a face！'
    if len(face_info_sor) > 1:
        print('The input image contain more than one face, we will only use the maximum face')
    ...
    dets_sor = face_info_sor['bbox']
    for img in imgs:
        cur_img_path = os.path.join(args.video_path, img)
        im_pil_sor = Image.open(cur_img_path).convert("RGB")
        bbox_pst_sor = get_bbox(dets_sor, crop_ratio=0.75)
        ...
        im_pil_sor = Image.fromarray(im_crop512_sor)
        im_pil_sor = pil2tensor(im_pil_sor)
        save_image(im_pil_sor, os.path.join(args.video_imgs_dir, img))
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--img_path",
        type=str,
        default='/home/mengting/projects/process_scripts/test_images/boy2.jpeg',
        required=False
    )
    parser.add_argument(
        "--save_path",
        type=str,
        default='/home/mengting/projects/process_scripts/test_images/boy2_cropped.jpg',
        required=False
    )
    # ********************************** driving video **************************************
    parser.add_argument(
        "--video_path",
        type=str,
        default='/home/mengting/projects/process_scripts/test_images/target_images1',
        required=False
    )
    parser.add_argument(
        "--video_imgs_dir",
        type=str,
        default='/home/mengting/projects/process_scripts/test_images/target_images1_cropped',
        required=False
    )
    args = parser.parse_args()
    if args.sign == 'image':
        crop_one_image(args)
    elif args.sign == 'video':
        crop_a_directory(args)
    else:
        assert 'sign is invalid'