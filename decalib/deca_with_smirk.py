...
from .smirk.mediapipe_utils import run_mediapipe
...
class DECA(nn.Module):
    def __init__(self, config=None, device='cuda'):
        ...
        checkpoint = torch.load(os.path.join(self.cfg.deca_dir, 'smirk/pretrained_models/SMIRK_em1.pt'))
    ...
    def load_smirk(self, imgpath):
        ...
        res_exp = outputs["expression_params"]
        res_jaw = outputs["jaw_params"]

        return res_exp, res_jaw
    ...
    def decode(self, codedict, rendering=True, iddict=None, vis_lmk=True, return_vis=True, use_detail=True,
               ...
               light_type='point', render_norm=False, imgpath=None):
        # images after cropped
        images = codedict['images']
        batch_size = images.shape[0]

        ## decode
        if imgpath is not None:
            new_exp, new_jaw = self.load_smirk(imgpath)
            codedict['exp'] = new_exp
            codedict['pose'][:, 3:] = new_jaw
        ...
        if rendering:
            ops = self.render(verts, trans_verts, albedo, codedict['light'], h=h, w=w,
                              add_light=add_light, th=th, light_type=light_type, render_norm=render_norm)
            ...
            opdict['depth_images'] = ops['depth_images']
